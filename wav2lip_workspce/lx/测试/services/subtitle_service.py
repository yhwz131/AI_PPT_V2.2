...
class SubtitleService:
    ...
    def load_whisper_model(self, model_size="large-v3"):
        ...
        try:
            ...
            self.whisper_model = None
            self.model_size = None
            fallback_order = {"large-v3": "medium", "medium": "small", "small": "base", "base": "tiny"}
            next_model = fallback_order.get(model_size)
            if next_model:
                print(f"尝试降级到 {next_model} 模型...")
                return self.load_whisper_model(next_model)
            return False
    ...
    def create_karaoke_subtitles_from_audio(self, audio_path, subtitle_path):
        ...
        try:
            ...
            if not self.load_whisper_model("large-v3"):
                print("⚠️ 所有 Whisper 模型加载失败，生成默认字幕")
                return self._generate_fallback_subtitle(audio_path, subtitle_path)
            ...
            for attempt in range(2):
                try:
                    ...
                    print(f"⚠️ Whisper 转录第 {attempt+1} 次失败: {e}")
                    try:
                        import torch
                        if torch.cuda.is_available():
                            torch.cuda.empty_cache()
                        ...
                        pass
                    if attempt == 0 and self.model_size and self.model_size != "small":
                        print("尝试降级模型后重试...")
                        self.whisper_model = None
                        self.model_size = None
                        self.load_whisper_model("small")
            
            if not result or not result.get('segments'):
                print("⚠️ Whisper 转录无结果，生成默认字幕")
                return self._generate_fallback_subtitle(audio_path, subtitle_path)
...
"""
    ...
    def _generate_fallback_subtitle(self, audio_path, subtitle_path):
        """Whisper 不可用时，用 ffprobe 拿时长后生成同款卡拉OK ASS 字幕"""
        try:
            from services.audio_processor import AudioProcessor
            duration = AudioProcessor().get_audio_duration(audio_path)
            font_size = self._calculate_dynamic_font_size(audio_path)

            fallback_text = "AI 知识讲堂"
            segment = [{'text': fallback_text, 'start': 0, 'end': min(duration, 5.0)}]
            ass_content = self._generate_improved_karaoke_ass(segment, duration, font_size)

            out_path = subtitle_path.replace('.srt', '.ass')
            with open(out_path, 'w', encoding='utf-8') as f:
                f.write(ass_content)
            print(f"⚠️ Whisper 不可用，已生成卡拉OK回退字幕: {out_path}")
            return True
        except Exception as e:
            print(f"❌ 回退字幕生成也失败: {e}")
            return False
